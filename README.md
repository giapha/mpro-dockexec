...
- **Re-execution (bounded proof of concept; 48 QC'd claims, 3 targets, AutoDock Vina 1.2.7):** against the engine's own near-deterministic run-to-run noise (within-ligand range median **0.04 kcal/mol**), the proximal driver of reproduction is **search-box availability** — claims run with a reported box size reproduce to a median **0.33 kcal/mol** versus **0.91** for those forced onto a 25 Å default, a **significant ~2.7-fold gap (Mann–Whitney p = 0.024)**. The executability class co-varies but is confounded with box-size source and underpowered (E3 n = 6, largely one paper; p = 0.07), so we read the label as an **executability gate** — which claims can be cleanly re-executed at all — not a reproducibility predictor.
